package com.crosslink.battleprism.core.misc;
...
public class ImageCreator {
	...
	public ImageCreator(ImageFormat ddsFormat, ImageSet.Dimensions ddsDimensions, int mipmapCount, int arrayCount, int faceCount) {
		m_imageFormat = ddsFormat;
		m_imageDimensions = new ImageSet.Dimensions(ddsDimensions);
		...
		for (int mipmapLevel = 0; mipmapLevel < mipmapCount; mipmapLevel++) {
			ImageSet.Dimensions mipmapLevelDimensions = Util.calcMipmapLevelDimensions(ddsDimensions, mipmapLevel);
	...
	public void setImageData(byte sourceData[], boolean isTopLeft, int mipmapLevel, int arrayIx, int faceIx) {
	...
	private ImageSet.Dimensions m_imageDimensions;
	...
	private void copyImageFlipped(byte[] sourceData, byte[] imageData, int imageDataOffset, int mipmapLevel) {		
		...
		ImageSet.Dimensions mipmapImageDimensions = Util.calcMipmapLevelDimensions(new ImageSet.Dimensions(m_imageDimensions), mipmapLevel);
		
		if (m_imageFormat.getPixelDataType().ordinal() < ImageFormat.PixelDataType.NUM_UNCOMPRESSED_TYPES.ordinal()) {
	...
	private void copyPixelsFlipped(ImageFormat imageFormat, 
			...
			int imageSize, ImageSet.Dimensions imageDimensions) {
	...
	private void copyBCFlipped(ImageFormat imageFormat,
			byte[] sourceData, byte[] imageData, int imageDataOffset, 
			int imageSize, ImageSet.Dimensions imageDimensions, int mipmapLevel) {
		// No support for 3D compressed formats.
		assert imageDimensions.numDimensions != 3 : "No support for 3D compressed formats.";

		Util.CompressedBlockData blockData = Util.getBlockCompressionData(imageFormat.getPixelDataType());
